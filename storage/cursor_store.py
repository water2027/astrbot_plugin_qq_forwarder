...
class IStore(ABC):
    ...
    @abstractmethod
    async def read_cursors(self) -> dict:
        pass
...
class FileStore(IStore):
    def __init__(self, data_dir: Path, target_groups: List[str]):
        ...
        self._ensure_files(target_groups)

    def _ensure_files(self, target_groups: List[str]):
...
class CursorStore:
    ...
    def __init__(self, store: IStore, capacity: int = 10):
        self.store = store
    ...
    async def update_cursor(self, group_id: str, msg_id: int):
        """更新指定源群的游标为 msg_id。"""
        async with self._lock:
            cursors = await self.store.read_cursors()
            cursors[group_id] = msg_id
            await self.store.write_cursors(cursors)
    ...
    async def remove_messages_up_to(self, msg_id: int):
        ...
        async with self._lock:
            ...
            await self.store.write_cache(cache[idx + 1 :])
